...
agregar_cliente() {
    ...
    if [ -n "$nombre" ]; then
        # Obtiene el último número de cliente y lo incrementa en uno
        ultimo_numero=$(awk -F"|" 'END {print $1}' "$ARCHIVO_CLIENTES")
        nuevo_numero=$((ultimo_numero + 1))
        
        # Agrega el nuevo cliente al archivo usr.txt
        echo "$nuevo_numero|$nombre" >> "$ARCHIVO_CLIENTES"
...
pagar_servicio() {
    ...
    if [ -n "$numero_cliente" ]; then
        if grep -q "^$numero_cliente|" "$ARCHIVO_CLIENTES"; then
            servicio=$(awk -F"|" '{print $1}' "$ARCHIVO_SERVICIOS" | zenity --list --title="Seleccionar Servicio" --column="Servicio")
            if [ -n "$servicio" ]; then
