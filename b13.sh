...
ver_saldo() {
    ...
    if [ -n "$clave_cliente" ]; then
        if grep -q "^$clave_cliente|" "$ARCHIVO_CLIENTES"; then
            ...
            zenity --info --title="Saldo del Cliente" --text="El saldo del cliente con clave primaria $clave_cliente es: $saldo"
        else
            zenity --info --title="Error" --text="La clave primaria $clave_cliente no fue encontrada."
        fi
    fi
    menu_principal
}
...
pagar_servicio() {
    clave_cliente=$(zenity --entry --title="Pagar Servicio" --text="Ingrese la clave primaria del cliente:")
    if [ -n "$clave_cliente" ]; then
        if grep -q "^$clave_cliente|" "$ARCHIVO_CLIENTES"; then
            servicio=$(zenity --list --title="Seleccionar Servicio" --column="Servicio" $(awk -F"|" '{print $1}' "$ARCHIVO_SERVICIOS"))
            if [ -n "$servicio" ]; then
                ...
                if [ "$saldo_actual" -ge "$precio_servicio" ]; then
                    ...
                    zenity --info --title="Pago Realizado" --text="El pago del servicio '$servicio' por $precio_servicio ha sido realizado correctamente para el cliente con clave primaria $clave_cliente."
                else
                    zenity --info --title="Error" --text="El cliente con clave primaria $clave_cliente no tiene saldo suficiente para realizar el pago del servicio."
                fi
            fi
        else
            zenity --info --title="Error" --text="La clave primaria $clave_cliente no fue encontrada."
...
deposito() {
    clave_cliente=$(zenity --entry --title="Depósito" --text="Ingrese el número del cliente:")
    if [ -n "$clave_cliente" ]; then
        ...
        if [ -n "$cantidad" ]; then
            if grep -q "^$clave_cliente|" "$ARCHIVO_CLIENTES"; then
                ...
                sed -i "s/^$clave_cliente|.*$/$clave_cliente|$nuevo_saldo|$(date +%Y-%m-%d)/" "$ARCHIVO_SALDOS"
                zenity --info --title="Depósito Realizado" --text="Depósito de $cantidad realizado correctamente para el cliente con número de cliente $clave_cliente."
            else
                zenity --info --title="Error" --text="El número de cliente $clave_cliente no fue encontrado."
            fi
        fi
    fi
    menu_principal
...
retiro() {
    clave_cliente=$(zenity --entry --title="Retiro" --text="Ingrese el número del cliente:")
    if [ -n "$clave_cliente" ]; then
        ...
        if [ -n "$cantidad" ]; then
            if grep -q "^$clave_cliente|" "$ARCHIVO_CLIENTES"; then
                ...
                if [ "$saldo_actual" -ge "$cantidad" ];then
                    nuevo_saldo=$((saldo_actual - cantidad))
                    sed -i "s/^$clave_cliente|.*$/$clave_cliente|$nuevo_saldo|$(date +%Y-%m-%d)/" "$ARCHIVO_SALDOS"
                    zenity --info --title="Retiro Realizado" --text="Retiro de $cantidad realizado correctamente para el cliente con número de cliente $clave_cliente."
                else
                    zenity --info --title="Error" --text="El cliente con número de cliente $clave_cliente no tiene saldo suficiente para realizar el retiro."
                fi
            else
                zenity --info --title="Error" --text="El número de cliente $clave_cliente no fue encontrado."
            fi
